...
class DataCollector:
    ...
    def update_mt5_credentials(self, credentials: Dict[str, Any]) -> bool:
        """Update MT5 credentials and reconnect"""
        try:
            self.mt5_config.update({
                'server': credentials.get('server', 'MetaQuotes-Demo'),
                'login': credentials.get('login'),
                'password': credentials.get('password'),
                'investor': credentials.get('investor')
            })
            ...
            self.logger.info(f"MT5 credentials updated, connected: {self.mt5_connected}")
            return self.mt5_connected
            
        except Exception as e:
            self.logger.error(f"Error updating MT5 credentials: {e}")
            return False
    
    def test_mt5_connection(self, credentials: Dict[str, Any]) -> bool:
        """Test MT5 connection without permanently updating credentials"""
        try:
            if not MT5_AVAILABLE or mt5 is None:
                self.logger.warning("MT5 not available for testing")
                return False
            
            if not mt5.initialize():
                self.logger.warning("MT5 initialization failed during test")
                return False
            ...
            if not login or not password:
                self.logger.warning("Missing login or password for test")
                return False
            
            test_result = mt5.login(login, password, server)
            
            mt5.shutdown()
            
            self.logger.info(f"MT5 connection test result: {test_result}")
            return test_result
